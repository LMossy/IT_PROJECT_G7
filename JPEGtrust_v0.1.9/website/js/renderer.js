...
// ─────────────────────────────────────────────────────────────
import { esc, dr, gpsToDecimal, formatGpsCoord } from "./utils.js";
...
export function buildDetailSections(file, mfst, exif, evalResult, sr) {
  ...
  if (hasExif) {
    ...
    // exifr may expose decimals as e.latitude/e.longitude and/or DMS arrays as GPSLatitude/GPSLongitude.
    const gpsLat = formatGpsCoord(
      e.latitude ?? e.GPSLatitude,
      e.GPSLatitudeRef ?? "",
    );
    const gpsLon = formatGpsCoord(
      e.longitude ?? e.GPSLongitude,
      e.GPSLongitudeRef ?? "",
    );
    const _gpsAlt = gpsToDecimal(e.GPSAltitude);
    const alt =
      _gpsAlt != null ? `${_gpsAlt.toFixed(1)} m` : null;
    const cs =
      e.ColorSpace != null
        ? e.ColorSpace === 1
          ? "sRGB"
...
function buildForensicTabsHTML(forensics, evalResult) {
  ...
  // Raw formats that the browser cannot render on canvas get an informative notice
  if (forensics.rawNotRenderable) {
    const label = forensics.rawInfo?.label ?? "RAW";
    return `
    <div class="forensic-raw-notice">
      <svg viewBox="0 0 20 20" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" style="flex-shrink:0;opacity:.7">
        <circle cx="10" cy="10" r="8"/>
        <path d="M10 6v4M10 13v.5"/>
      </svg>
      <div>
        <strong>Pixel-level forensics unavailable for ${esc(label)} format</strong>
        <p style="margin:.35rem 0 0;opacity:.8">
          The browser cannot render this raw file on a canvas for pixel analysis.
          EXIF metadata analysis above still applies. For full pixel forensics, convert
          the file to JPEG or TIFF first using dedicated raw-processing software.
        </p>
      </div>
    </div>`;
  }
  ...
  const elaPanel = _forensicPanel(
    ...
    ela.visualURL,
    ela.isLossless
      ? ela.sourceFormat && !/lossless/i.test(ela.sourceFormat)
        ? `\u26a0 Raw source (${ela.sourceFormat}): higher baseline ELA is normal \u2014 results are indicative only.`
        : "\u26a0 Lossless source (PNG/WebP/RAW): higher baseline ELA is normal \u2014 results are indicative only."
      : null,
    [
