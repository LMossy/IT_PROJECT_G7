...
function onFile(file) {
  ...
  // Determine display type label (handles raw formats with empty file.type)
  const rawInfo = getRawInfo(file);
  const typeLabel = rawInfo ? rawInfo.label : file.type || "unknown";

  $pnm.textContent = file.name;
  $pmeta.textContent = `${(file.size / 1024).toFixed(1)} KB · ${typeLabel} · ${new Date(file.lastModified).toLocaleDateString()}`;

  // Load preview — for HEIC/raw try to get a renderable version
  (async () => {
    try {
      const { dataURL } = await getDisplayableDataURL(file, Exifr);
      currentDataURL = dataURL;
      $pim.src = dataURL;
    } catch (_) {
      // Fallback: plain FileReader (may show blank for unrenderable raw in non-Safari)
