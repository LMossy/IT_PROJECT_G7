 ...
 */
export async function runForensics(dataURL, mimeType) {
  let img;
  try {
...
async function _runELA(img, mimeType) {
  const w = img.naturalWidth;
  const h = img.naturalHeight;
  const isLossless = /png|webp|bmp|gif/.test((mimeType || "").toLowerCase());
  ...
  return {
    ...
    isLossless,
    sourceFormat: isLossless ? "Lossless (PNG/WebP)" : "JPEG",
    risk,
    verdict,
    verdictType,
  };
